        ...
        jQuery(function() {
            ...
            function GetCoordinates(layer, resultsArray, places, hasMore, veErrorMessage) {
                ...
                if (geoinfos.length == 1)
                    mainMap.SetCenterAndZoom(places[0].LatLong, 8);
         ...
         streetaddress, region, locality, postalcode, countryname, photo, workphone, workfax, homephone, cellphone, pager, url, bday, latitude, longitude, category, rev, uid, mailer) {
            ...
            function displayUI() {
                ...
                if (colColumnCount == 1 && geoinfos.length == 0 && addresses.length == 0) {
                    ...
                    iwmf.css({ 'width': '1460px' });
                    width = "1460px";
                    oomph('.iwmf_box').css({ 'width': '1400px', 'border-right': '0px' });
                    oomph('.iwmf_mainNavContainer').css({ 'width': '1400px', 'border-right': '0px' });
                    oomph('.iwmf_hmediaphoto').css({ 'max-width': '1400px' });
                }
                // if we have 1 mf and 1 map -- 550px px
                if (colColumnCount == 1 && (geoinfos.length > 0 || addresses.length > 0)) {
                    iwmf.css({ 'width': '1550px' });
                    width = "1550px";
                }
            ...
            function findAndParseHAudio() {
                ...
                oomph(hAudioCollection).each(function() {
                    ...
                    if (enclosure) {
                        ...
                        oomph(hAudioUI).append('<object data="data:application/x-silverlight," type="application/x-silverlight-2" width="1200" height="' + height + '"><param name="source" value="http://visitmix.com/labs/oomph/2.0/Inplay.xap" /><param name="enableHtmlAccess" value="true"   /> <param name="background" value="#00000000" /><param name="windowless" value="true" /><param name="initParams" value="id=Video,source=' + qualifyHREF(enclosure) + ',background=50AAAAAA,volume=0.5,play=false" /><a href="http://go.microsoft.com/fwlink/?LinkID=124807"><img src="http://go.microsoft.com/fwlink/?LinkId=108181" alt="Get Microsoft Silverlight" /></a></object>  ');
                    }
                    if (photo)
